...
class Word_Embedding_Model_Creator_Unittest(unittest.TestCase):
    ...
    def test_where_clause_effect(self):
        ...
        self._setup_test()
        db_results = self._db.get_author_word_embedding('0', 'target_article_items', "_type_paragraph_content") # check if this is ok with Aviad
    ...
    def test_add_additional_fields_to_existing_table(self):
        ...
        self._word_embedding_model_creator.execute(None)
        db_results = self._db.get_author_word_embedding('test_user', 'posts','title')  # check if
        #  this is ok with Aviad
        try:
            if db_results['sum'] is not None and db_results['np.mean'] is not None:
    ...
    def _generic_test(self, expected_value, source_id=""):
        ...
        db_results = self._db.get_author_word_embedding(source_id, 'posts', 'content')
        self.assertEqual(expected_value['min'], db_results['min'])
        self.assertEqual(expected_value['max'], db_results['max'])
        self.assertEqual(expected_value['np.mean'], db_results['np.mean'])
