...
class PyDictionary(object):
    ...
    def printMeanings(self):
        ...
        for key in list(dic.keys()):
            ...
            for k in list(dic[key].keys()):
                print(k + ':')
                for m in dic[key][k]:
                    print(m)
    def printAntonyms(self):
        antonyms = dict(list(zip(self.args,self.getAntonyms(False))))
        for word in antonyms:
            print(word+':')
            print(', '.join(antonyms[word]))
    def printSynonyms(self):
        synonyms = dict(list(zip(self.args,self.getSynonyms(False))))
        ...
        if len(term.split()) > 1:
            ...
            try:
                ...
                if types[0].text == 'Your search did not return any results.':
                    return {'temp': None}
                if types[0].text == 'Sorry, your search can only contain letters, numbers, spaces, hyphens, periods, slashes, and/or apostrophes.':
                    return {'temp': None}
                length = len(types)
